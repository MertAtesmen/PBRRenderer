...
inline void PbrRenderer::run()
{
    ...
    PBR::Shader texture_maps_shader = shaders["texture_maps_shader"];
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        context.material = contexts[material_index];

        // Draw the sphere
        if(!show_texture_maps)
        {
        ...
        else
        {
            ...
            texture_maps_shader.use();

            texture_maps_shader.setMat4("view", view);
            texture_maps_shader.setMat4("projection", projection);
            texture_maps_shader.setMat4("model", glm::mat4{ 1.0f });
            ...
            texture_maps_shader.setInt("texture1", 0);
        ...
        if (ImGui::BeginCombo("##1", materials[material_index])) {
            for (int n = 0; n < IM_ARRAYSIZE(materials); n++) {
                bool is_selected = (material_index == n);
        ...
        ImGui::Checkbox("Show Texture Maps", &show_texture_maps);


        if(show_texture_maps)
        {
            ImGui::Text("Textures");
            if (ImGui::BeginCombo("###2", texture_maps[texture_index])) 
            {
                for (int n = 0; n < IM_ARRAYSIZE(texture_maps); n++) 
                {
                    bool is_selected = (texture_index == n);
                    if (ImGui::Selectable(texture_maps[n], is_selected))
                        texture_index = n;
                    if (is_selected)
                        ImGui::SetItemDefaultFocus();
                }
...
inline void PbrRenderer::_load_GL_textures()
{
    ...
    // _load_GL_material("gold");
    // _load_GL_material("plastic");
    // _load_GL_material("rusted_iron");
}
